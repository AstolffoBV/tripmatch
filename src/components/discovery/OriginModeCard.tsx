...
export default function OriginModeCard({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`group relative min-h-28 w-full overflow-hidden rounded-[1.45rem] border px-4 py-4 text-left shadow-sm transition duration-200 focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#1b7c83] motion-reduce:transform-none motion-reduce:transition-none sm:px-5 dark:focus-visible:outline-[#83d9d2] ${
        disabled
          ...
          : selected
            ? "cursor-pointer border-[#55c9c5] bg-[#123f46] text-[#f5fffd] shadow-[0_17px_40px_rgba(12,70,76,0.18)] motion-safe:hover:-translate-y-0.5 hover:border-[#76ddd7] hover:shadow-[0_18px_42px_rgba(12,70,76,0.21)]"
    ...
    >
      ...
      <span className="relative flex h-full items-center gap-4">
        ...
        <span
          aria-hidden="true"
          className={`absolute top-0 right-0 grid size-6 place-items-center rounded-full border transition duration-200 motion-reduce:transform-none motion-reduce:transition-none ${
            selected
